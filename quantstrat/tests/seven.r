...
stats.seven  = tradeStats(portfolio1.st)

txns7       = last(stats.seven$Num.Txns)
trades7     = last(stats.seven$Num.Trades)
maxeq7      = last(stats.seven$Max.Equity)
maxdraw7    = last(stats.seven$maxDrawdown)
largwin7    = last(stats.seven$Largest.Winner)
...
test_that('Max equity is consistent', {
  ...
  expect_that(maxeq7, is_equivalent_to(3877.741))
})
...
test_that('Max drawdown is consistent', {
  ...
  expect_that(maxdraw7, is_equivalent_to(-30979.74))
})
